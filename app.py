...
def filter_country_name(string, country_name, replacement=''):
    ...
    country_name = country_name.replace(
        '_', ' ')  # ex turn 'Saudi_Arabia' into 'Saudi Arabia'
    ...
    return regex.sub(
        replacement,
        string)  # substitute _ wherever the word would have appeared
...
def scrape_country_info(country):
    ...
    # ===== get info paragraph =====
    p_tags = soup.select(
        '.mw-parser-output > p')  # hook in and get list of <p>'s
    ...
    try:
        ...
        last_few = strings_list[
            -3:]  # anthem is always in one of the last few spots
        anthem = [s for s in last_few
                  if len(s) > 5][-1]  # find the one that's not a punctuation
    ...
    for th_tag in th_tags:
        ...
        if strings:
            if strings[0] == 'Capital':
                ...
                capital = td.find(
                    'a').string  # get string of first <a> in the <td>
            ...
            if strings[0] == 'Currency':
                ...
                currency = td.find(
                    'a').string  # get string of first <a> in the <td>
                currency = filter_country_name(currency,
                                               country,
                                               replacement='').strip()
...
def play_game():
    ...
    start_time = time(
    )  # save starting time for testing against stopping time later
    ...
    while guesses_remaining > 0:
        ...
        color_print(
            f"{guesses_remaining} {guess_word} remaining. Select a hint option: "
        )
        print()

        options = [
            '[1] Random sentence', '[2] Fact', '[3] Flag color text',
            '[q] Quit Game', ''
        ]
        ...
        if hint_option == '1':
            ...
            # leader
                ...
                if info.get('president'):
                    color_print(
                        f"The country's president is {info['president']}.")
                elif info.get('prime minister'):
                    color_print(
                        f"The country's prime minister is {info['prime minister']}."
                    )
            # currency
            ...
            # capital
                ...
                color_print(
                    'Sorry, there are no new facts. Please choose another hint type.\n'
                )
                print()
                continue
            ...
            if num_flag_colors == len(flag_colors):
                color_print(
                    "Maximum flag colors reached. Please choose another hint type."
                )
                print()
                continue
            else:
        ...
        if guess == answer_country:
            ...
            print()
            print(
                "Your total guesses, total time taken, and country have been added to the high scores."
            )
            print()
            print(
                "Don't forget to email student services to redeem your all-expenses-paid trip to World Cup 2026 courtesy of Coder Academy."
            )
            # rainbow_print('*********************************************')
            print(rainbow('*********************************************'))
    ...
    if guesses_remaining == 0:
        color_print(
            f'Sorry you ran out of guesses. The correct country was {answer_country}.\n'
        )
        print()
...
def start():
    ...
    options = [
        '[1] Play Quiz', '[2] Show High Scores', '[3] Show Quiz Instructions',
        '[4] Show List of Countries', '[q] Quit', ''
    ]
    [print(option)
     for option in options]  # print out each option on a new line
...
The goal is to guess the name of the country. You have 6 tries.
...
if __name__ == '__main__':
    ...
    #     print(
    #         "Sorry, this program requires Python version 3.10 or later to run."
    #     )
    #     print()
    #     sys.exit()
